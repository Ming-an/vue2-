          ...
          <div class="choose">
            ...
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in spuSaleAttrList" :key="item.id">
                ...
                <dd v-for="(value, index) in item.spuSaleAttrValueList" :key="value.id"
                  :class="{ active: value.isChecked == 1 }" @click="changeActive(value, item.spuSaleAttrValueList)">{{
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      //如果服务器数据没有回来 skuInfo 这个对象是一个空对象
  ...
  methods: {
    ...
    async addShopcart() {
      /* 
        当前请求这里是派发一个action 也想服务器发请求 判断加入购物车是否成功进行了相应操作

      */
      // let res = await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum});
      // console.log(res) 返回的是一个promise
      // promise 要么返回成功要么失败
      try {
